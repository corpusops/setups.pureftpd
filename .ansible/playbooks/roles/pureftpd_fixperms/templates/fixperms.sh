#!/usr/bin/env bash
# {{ansible_managed}}{% set cfg = cops_pureftpd_vars %}
...
# {% for users  in cfg.ftp_users %}
# {% for ftp_user, fdata in users.items() %}
# {% set home        = fdata.home %}
# {% set operms      = fdata.get('operms', 'rwx') %}
# {% set perms       = fdata.get('perms',  'rwx') %}
# {% set owner       = fdata.owner      |default(ftp_user.user |default(cfg.user )) %}
# {% set owner_group = fdata.owner_group|default(ftp_user.group|default(cfg.group)) %}
# {% set user        = fdata.user |default('nobody') %}
# {% set group       = fdata.group|default('nobody') %}
if [ -e "{{home}}" ];then
    setfacl -PR \
     -m "u:{{owner}}:{{operms}},g:{{owner_group}}:{{operms}},d:u:{{owner}}:{{operms}},d:g:{{owner_group}}:{{operms}}" \
     "{{home}}"
    setfacl -PR \
     -m "u:{{user}}:{{perms}},g:{{group}}:{{perms}},d:u:{{user}}:{{perms}},d:g:{{group}}:{{perms}}" \
     "{{home}}"
fi
# {% endfor %}
# {% endfor %}
